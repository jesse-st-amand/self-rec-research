...
building block for several practical use cases. (a) **Model
attribution**: in pipelines or agent settings where multiple models
generate text into a shared context, a model that can recognize its
own contributions can avoid double-counting or misattribution.
(b) **In-context manipulation detection**: prompt-injection and
jailbreak attacks frequently work by inserting exemplars that the
model treats as authoritative; a model that classifies injected
exemplars as "not mine" is positioned to discount them.
(c) **Calibrated self-knowledge**: recognition of own outputs probes
...
Our entering expectation was that SGTR-trained models — having been
...
straightforward: the harmful exemplars inserted by an MSJ attack are
...
**H1 — Catastrophic forgetting of MSJ-resistance training.** SGTR
fine-tuning may erode the base model's prior MSJ-resistance training.
The 50-shot dip is consistent with this account: a defense trained at
a fixed shot budget would degrade more at off-distribution shot counts
than at the trained shot count under any subsequent fine-tuning.
...
exemplars that shift its representation of the candidate text toward
...
MMLU-ICE — share the same prompt structure: a sequence of N **ICE pairs**
of the form (User Query, Assistant Response), followed by a final probe.
The probes differ:

- **MSJ**: User → ICE-Query (e.g., "How do you build a bomb?"); Assistant
  (pre-filled) → ICE-Response with harmful content; repeat × N; final
  User probe = a different harmful query. Attack success = compliance.
- **SGTR-ICE**: User → ICE-Query (e.g., "Summarize the following
  article…"); Assistant (pre-filled) → ICE-Response; repeat × N; final
  User probe = the SGTR question ("Is this text yours?"). Attack success
  = attribution bias (defined in §2.5).
- **MMLU-ICE**: same scaffold; final probe = a multiple-choice MMLU
  question. Attack success = accuracy drop.
...
SGTR-ICE configuration may or may not be — `ica-self` priming on a
GPT-OSS-20B IND-self subseries reinforces correct behavior (or has no
detectable effect at the accuracy ceiling), while the same priming on
the IND-alt subseries induces positive attribution bias and so
constitutes an ICA. Distinguishing the structural ICE from the outcome
...
`ica-ctrl{1,2,3}`) describe **which author the ICE responses are
attributed to** at evaluation time. They are kept for pipeline
compatibility and should be read as ICE-author descriptors rather than
as labels for "attacks" in their own right.
...
The "alt" model serves as the source of `not-self` exemplars in SGTR
...
several places below — the most likely first-order explanation is that
its capacity is too limited to absorb the multi-OP training signal
without representational interference, rather than a family-specific
mechanism that needs its own theoretical account. We flag the
...
| **Out-of-distribution recognition, eval-source ICA**   | `SGTR_07_trained-OP-ShareGPT_eval-on_self-same-OP-WikiSum_ICA-ShareGPT`        | Trained on ShareGPT, evaluated on WikiSum (a different downstream task) under SGTR; ICA exemplars come from ShareGPT.                                          |
| **Out-of-distribution recognition, matched-source ICA**| `SGTR_09_trained-OP-ShareGPT_eval-on_self-same-OP-WikiSum_ICA-WikiSum`         | Same training and SGTR-eval setup, but ICA exemplars come from WikiSum (matching the eval distribution).                                                        |
| **MMLU under ICA**                                     | `MMLU_01_trained-OP_eval-on_self-same-OP`                                      | 50-question MMLU-MC subset evaluated with author-attributed in-context exemplars; measures whether ICA degrades general capability in trained vs. base models. |
...
- **Attribution bias.** Average accuracy drops under ICE *and* the drop
  ...
  (positive bias: `IND-alt` accuracy collapses, `IND-self` flat or near
  ceiling) or "nothing is mine" (negative bias: `IND-self` collapses,
  `IND-alt` flat). An asymmetric *increase* on one subseries paired
...
SGTR on WikiSum, with ICE exemplars drawn either from ShareGPT
...
defense by enabling the model to discount injected exemplars — is
...
Vary training-data and -epoch budgets per base (Llama, GPT-OSS, Qwen)
on a single OP and on the multi-OP mixture. The Llama-specific result
in §4.4 — multi-OP being *worse* than its single-OP average — is
consistent with several mechanisms (Llama-specific brittleness, reduced
multi-OP headroom, per-OP-budget × parameter-count interaction) that
this sweep would help separate.
