...
# Read REASONING_VS_INSTRUCT from YAML config
REASONING_VS_INSTRUCT=$(python3 -c "
import yaml
with open('$CONFIG_FILE') as f:
    config = yaml.safe_load(f)
val = config.get('reasoning_vs_instruct', False)
...
EXTRA_ARGS=()
if [[ -n "${REASONING_VS_INSTRUCT:-}" ]] && [[ "$REASONING_VS_INSTRUCT" != "0" ]]; then
    EXTRA_ARGS+=(--reasoning_vs_instruct)
fi
