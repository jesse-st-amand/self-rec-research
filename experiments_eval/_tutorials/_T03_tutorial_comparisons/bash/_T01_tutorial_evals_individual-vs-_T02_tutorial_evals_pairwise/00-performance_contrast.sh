...
# ============================================================================
# Load configuration from the comparison config.yaml (shared loader)
# ============================================================================
...
# The aggregated dir is named after the experiment. The comparison name may be
# the FULL experiment name (tutorial: exact dir) or a short PREFIX (e.g. ICML_01
# -> ICML_01_UT_PW-...). Try exact match first, then a prefix glob.
EXP1_EXP_DIR="$EXP1_BASE_DIR/$EXP1"
if [[ ! -d "$EXP1_EXP_DIR" ]]; then
    EXP1_EXP_DIR=$(ls -d "$EXP1_BASE_DIR/$EXP1"_* 2>/dev/null | head -1)
fi

if [[ -z "$EXP1_EXP_DIR" ]] || [[ ! -d "$EXP1_EXP_DIR" ]]; then
    echo "Error: No experiment directory found for: $EXP1"
    echo "  Searched in: $EXP1_BASE_DIR (exact '$EXP1' or prefix '${EXP1}_*')"
    exit 1
fi
...
# Find experiment directory for exp2 (exact match first, then prefix glob)
EXP2_EXP_DIR="$EXP1_BASE_DIR/$EXP2"
if [[ ! -d "$EXP2_EXP_DIR" ]]; then
    EXP2_EXP_DIR=$(ls -d "$EXP1_BASE_DIR/$EXP2"_* 2>/dev/null | head -1)
fi

if [[ -z "$EXP2_EXP_DIR" ]] || [[ ! -d "$EXP2_EXP_DIR" ]]; then
    echo "Error: No experiment directory found for: $EXP2"
    echo "  Searched in: $EXP1_BASE_DIR (exact '$EXP2' or prefix '${EXP2}_*')"
    exit 1
fi
